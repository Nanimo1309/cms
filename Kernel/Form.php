<?php
	class Form
	{
		...
		public function __construct($action, $name, $attr = [])
		{
			$this->actionn = $action;
			$this->name = $name;
		...
		{
			echo "<form id='$this->name'" . self::writeAttrib($this->attr) . '>';
		...
		final public function write($name)
		{
			...
			case 'textarea':
				...
				echo "<textarea id='$id' name='$name'" . self::writeAttrib($in['attrib']) . '></textarea>';
				break;
			default:
				if($in['labelText'])
					echo "<label id='$id-label' for='$id'" . self::writeAttrib($in['labelAttrib']) . ">{$in['labelText']}</label>";
